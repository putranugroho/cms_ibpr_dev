...
import 'package:cms_ibpr/module/limit/limit_trx_notifier.dart';
...
class LimitTrxPage extends StatelessWidget {
  const LimitTrxPage({super.key});

  @override
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(
      create: (_) => LimitTrxNotifier(context: context),
      child: Consumer<LimitTrxNotifier>(
...
class DetailDataSource extends DataGridSource {
  DetailDataSource(List<LimitTrxModel> list) {
    int index = 1;
    _laporanData = list.map<DataGridRow>((data) {
      final row = DataGridRow(
        cells: [
          ...
          DataGridCell(columnName: 'tarik_tunai', value: (data.trkTunaiTrx ?? "0").toString()),
          DataGridCell(columnName: 'setor_tunai', value: (data.setorTrx ?? "0").toString()),
          DataGridCell(columnName: 'transfer', value: (data.trfTrx ?? "0").toString()),
          DataGridCell(columnName: 'ppob', value: (data.ppobTrx ?? "0").toString()),
          DataGridCell(columnName: 'qr', value: (data.qrTrx ?? "0").toString()),
        ],
      );
      index++;
      return row;
